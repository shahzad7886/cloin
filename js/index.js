...
(function () {
  ...
  var pageAttributes = {
    ...
    app_version: normalizeAttributeValue(
      "0b7f9829542963ed6d1ca3a3ba52645061629708"
    ),
    page_type: normalizeAttributeValue("item"),
    ga_param: normalizeAttributeValue(
      "_ga=2.228978527.238600286.1719175142-1585676727.1718013449"
    ),
...
function setExperimentEnrollments(experimentEnrolmentsDataString) {
  for (var i = 0; i < ACCOUNTS.length; i++) {
    ...
    if (
      cookieValue &&
      cookieValue.replace(/\*/g, "!") === experimentEnrolmentsDataString
    ) {
...
function currentSiteLinkOpensInNewWindow(link) {
  return (
    document.location.hostname === link.hostname && link.target === "_blank"
  );
}
    ...
    var e = {
      ...
      _envTrackevent: function (e, t, n, r) {
        for (var i = 0; i < this._envTrkrs.length; i++) {
          var s =
            this._envTrkrs[i][0].length == 0 ? "" : this._envTrkrs[i][0] + ".";
          r.length == 0
      ...
      _envTrackpageview: function (e, t) {
        ...
        for (var n = 0; n < this._envTrkrs.length; n++) {
          var r =
            this._envTrkrs[n][0].length == 0 ? "" : this._envTrkrs[n][0] + ".";
      ...
      _envLinksTracker: function (t) {
        ...
        if (o.match(/^#/)) {
          if (i.self.run) {
            i.self.useEvent
              ? e._envTrackevent("self", "click - " + c, o, "")
              : e._envTrackpageview("/virtual/self", "/" + o);
            return true;
          }
          ...
          if (i.mail.run) {
            ...
            i.mail.useEvent
              ? e._envTrackevent("mailto", "click - " + c, o, "")
              : e._envTrackpageview("/virtual/mailto", o);
            return true;
          }
          ...
          if (i.download.run) {
            o = o.replace(/^https?:\/\//i, "");
            i.download.useEvent
              ? e._envTrackevent("download", "click - " + c, o, "")
          ...
          if (e.isInArray(u, e._envArray) == -1) {
            if (i.outbound.run) {
              i.outbound.useEvent
                ? e._envTrackevent("outbound", "click - " + c, u + a + l, "")
                : e._envTrackpageview("/virtual/outbound", u + a);
              return true;
            }
            ...
            if (h != null && h == "_blank") {
              ...
              var p =
                e._envTrkrs[0][0].length == 0 ? "" : _envTrkrs[0][0] + ".";
              return true;
            } else {
              return false;
            }
